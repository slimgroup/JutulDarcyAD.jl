...
using Jutul: simulate, setup_parameter_optimization, optimization_config, vectorize_variables
using JutulDarcy
...
function ChainRulesCore.rrule(::typeof(simulate_ad), state0, model, tstep, parameters, forces; opt_config=nothing, kwargs...)
    ...
    function simulate_ad_pullback(doutput)
        ...
        function F(model, state_ad, dt, step_no, forces)
            ...
            T = eltype(state_ad.Saturations[1,1])
        ...
        opt = setup_parameter_optimization(model, state0, parameters, tstep, forces, F, opt_config, print = 100, param_obj = true);
